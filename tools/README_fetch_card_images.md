...
M6aは、公式カード検索に個別掲載されないシークレットカード、LEGEND下側、基本エネルギー、RGBミュウ3種があるため、通常の画像取得後に専用補完ツールを実行します。

```powershell
py tools/complete_m6a_card_master.py --sleep 0.2
```

このツールはM6aだけを対象とし、公開カードショップ一覧とRGBミュウを掲載する公開記事から不足分のカード名と画像を補完します。また、公式検索では上下連結画像になるLEGEND 151/103を上側単体画像へ差し替えます。通常カードと復刻カードの大部分には、公式カード検索から取得した画像を引き続き使用します。取得元と権利関係を確認してから実行してください。
